# Taken from: https://github.com/n8n-io/n8n/tree/n8n/docker/images/n8n-custom
...
ARG VUE_APP_URL_BASE_API="http://localhost"
...
RUN VUE_APP_URL_BASE_API=${VUE_APP_URL_BASE_API} pnpm build
...
RUN mkdir /compiled
RUN NODE_ENV=production VUE_APP_URL_BASE_API=${VUE_APP_URL_BASE_API} DOCKER_BUILD=true pnpm --filter=n8n --prod --no-optional --legacy deploy /compiled
...
COPY --from=builder /src/docker/images/n8n/docker-entrypoint.sh /docker-entrypoint.sh
...
COPY --from=builder /src/docker/images/n8n/n8n-task-runners.json /etc/n8n-task-runners.json
